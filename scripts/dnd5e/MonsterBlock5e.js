...
export default class MonsterBlock5e extends dnd5e.applications.actor.ActorSheet5eNPC {
	...
	*/
	...
	prepSenses(data) {
		...
		for (let sense of senseTypes) {
			...
			senses.push({
				...
				unit: (data.system.attributes.senses.units || "ft") + game.i18n.localize("MOBLOKS5E.SpeedUnitAbbrEnd"),
	...
	_onChangeInput(event) {
		...
		if (input.dataset.entity) {		
			...
			this.actor.update({[key]: value}).then(()=> { super._onChangeInput(event); });
		}
	}
   ...
   */
	...
	static handlebarsHelpers = {
		...
		"moblok-split-clang": (obj) => { // Check if an array is empty.
			return obj.split(";")
		},
